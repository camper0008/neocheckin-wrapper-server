...
import { downloadAllEmployees, downloadAllEmployeesWithImages } from "../employees/downloadEmployees";
...
async (req, res) => {
  try {
    res.status(200).json({data: (await downloadAllEmployees(db, idb)).map(e => ({...e, rfid: e.rfid.toString()}))});
  } catch (catched) {
...
async (req, res) => {
  try {
    res.status(200).json({data: (await downloadAllEmployeesWithImages(db, idb)).map(e => ({...e, rfid: e.rfid.toString()}))});
  } catch (catched) {
    res.status(500).json({error: 'server error'});
    console.error(catched);
  }
